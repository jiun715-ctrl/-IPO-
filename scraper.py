...
# 10 × 3섹션 + 헤더/디바이더/컨텍스트 ≈ 최대 40개 block
MAX_ITEMS_PER_SECTION = 10


def _format_item_full(it: IpoItem) -> str:
    """공모중·최근마감: 6컬럼 모두 세로 bullet."""
    lines = [
    ...
    # 청약경쟁률은 보통 마감 후에만 값이 있음 → 공모중엔 '-'로 표기
    comp = it.competition if it.competition else "-"
    lines.append(f"• 청약 경쟁률 : {comp}")
    lines.append(f"• 주간사 : {it.underwriter}")
    return "\n".join(lines)
...
def build_blocks(
    ...
    blocks: list[dict] = [
        {
            "type": "header",
            "text": {"type": "plain_text", "text": f"경쟁사 IPO 일정 ({run_date})"},
        },
        {"type": "divider"},
    ]
